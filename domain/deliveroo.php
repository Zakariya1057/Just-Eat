...
class Deliveroo {
    ...
    public function postcodes(){
        ...
        for ($i = 1; $i <= 99;$i++){
            ...
            if(sizeof($restaurant) !== 0){
                ...
                $path = "postcodes".DIRECTORY_SEPARATOR."$postcode_area$i-$city.html";
                $output[] = $path;
    ...
    public function menu($url) {
        ...
        // file_put_contents('restaurants'.DIRECTORY_SEPARATOR.$matches[1]."_menu.html",$html);

        //////////////////////////////////////////////////////////////////////////////////////////////

        $html = file_get_contents('restaurants/restaurants-caspian-grill-and-pizza-birmingham.html');
        $crawler = new Crawler($html);
        ...
        $crawler->filter('.category')->each(function(Crawler $node, $i){
            ...
            $node->filter('h3')->each(function(Crawler $node, $i){
                global $category;
                $category->name = $this->shorten($node->text());
            });
    ...
    public function info($url){
        ...
        if (sizeof($crawler->filter('.restaurantOverview ')) !== 0) {
            ...
            file_put_contents('restaurants'.DIRECTORY_SEPARATOR.$matches[1]."_info.html",$html);
...
END;
    ...
    public function insert($restaurant){
        ...
        file_put_contents("logos".DIRECTORY_SEPARATOR."$online_id.gif", fopen($logo, 'r'));
    ...
    public function new($new_restaurants){
        ...
        file_put_contents("$city.json",$list);
